...
import { categories, getCapsulesBySection, getLocalizedCategoryName, getLocalizedSectionName, getLocalizedDifficulty } from "@/data/capsules";
...
const subjectGradient: Record<string, string> = {
  biology: "from-green-500 to-emerald-600", math: "from-blue-500 to-indigo-600",
  ukrainian: "from-yellow-500 to-amber-600", english: "from-red-500 to-rose-600",
  history: "from-orange-500 to-amber-700", chemistry: "from-purple-500 to-violet-600",
      ...
      <div className={`pt-24 pb-10 bg-gradient-to-br ${gradient} relative overflow-hidden`}>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <Link to="/categories" className="inline-flex items-center gap-2 text-sm text-white/80 hover:text-white transition-colors mb-6">
            <ArrowLeft className="w-4 h-4" /> {t.allCategories}
          </Link>
          <div className="flex items-center gap-4">
            <span className="text-5xl drop-shadow-lg">{category.icon}</span>
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-white">{getLocalizedCategoryName(category, lang)}</h1>
              <p className="text-white/80 text-sm mt-1">{category.count} {t.capsules} · {category.sections.length} {t.sections}</p>
            </div>
          </div>
        </div>
      </div>
      <div className="pb-16">
        <div className="container mx-auto px-4">
          <div className="bg-card rounded-xl border border-border shadow-sm p-4 -mt-5 relative z-10 mb-8 flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-muted-foreground mr-2">{t.difficulty}</span>
            {difficulties.map(d => (
              <button key={d} onClick={() => setDifficulty(d)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  difficulty === d ? "bg-primary text-primary-foreground shadow-sm" : "bg-muted text-muted-foreground hover:text-foreground hover:bg-muted/80"
                }`}>
                {d === "усі" ? t.allLevels : getLocalizedDifficulty(d, lang)}
              </button>
            ))}
          </div>
          {category.sections.map((section, si) => {
            let items = getCapsulesBySection(category.id, section.id);
            if (difficulty !== "усі") items = items.filter(c => c.difficulty === difficulty);
            if (items.length === 0) return null;
            return (
              <motion.div key={section.id} initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-40px" }} transition={{ delay: si * 0.05, duration: 0.5 }} className="mb-12">
                <h2 className="text-xl font-bold mb-5 flex items-center gap-3">
                  ...
                  {getLocalizedSectionName(section, lang)}
