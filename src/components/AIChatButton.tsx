...
const AIChatButton = ({ topicTitle, topicContext }: AIChatButtonProps) => {
  ...
  const suggestedQuestions = [
    `Поясни "${topicTitle}" ще простіше`,
    `Наведи ще приклади до "${topicTitle}"`,
    `Як це використовується в житті?`,
  ];
  ...
  const sendMessage = async (text: string) => {
    ...
    // Simulate AI response (replace with real AI call when backend is ready)
    setTimeout(() => {
      setMessages(prev => [...prev, {
        role: "ai",
        text: `Дякую за запитання про "${topicTitle}"! На жаль, AI-асистент зараз працює в демо-режимі. Підключіть Lovable Cloud для повноцінної роботи AI. 🤖`
      }]);
      setLoading(false);
    }, 1200);
  };
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-primary/10 to-secondary/10 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm font-semibold text-foreground">AI Помічник</div>
              ...
              {messages.length === 0 && (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground text-center mb-4">
                    Запитай будь-що про тему «{topicTitle}» 🧠
                  </p>
